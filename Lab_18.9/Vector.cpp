...
Vector::Vector(int s, int k)
{
	if (s > MAX_SIZE)
	{
		throw error("Äëèíà âåêòîðà áîëüøå ìàêñèìàëüíîãî ðàçìåðà\n");
	}
	if (s < 0)
	{
		throw error("Äëèíà âåêòîðà ìåíüøå íóëÿ\n");
	}
...
{
	if (s > MAX_SIZE) throw error("Äëèíà âåêòîðà áîëüøå ìàêñèìàëüíîãî ðàçìåðà\n");
	size = s;
	beg = new int[size];
	for (int i = 0; i < size; i++)
		beg[i] = mas[i];
